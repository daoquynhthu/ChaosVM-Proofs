...
| **G2** | T14 联合发散 — 当前只证明 per-channel（σ/CFA/DDM/H 各自链单射），未证明任一通道差异 → 所有后续状态差异 | 3–4h | P2 | ⏳ 待做 |
| **G3** | 分支纠缠正确性 — Lean 模型显式抽象掉 B=3 分支纠缠；需建模 shadow register 独立性 + real_idx 选择 | 4–6h | P3 | ⏳ 待做 |
