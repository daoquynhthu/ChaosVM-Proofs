...
- 单轮可逆 ✅ 已证明
- 三轮可逆 🏗 需要定义 `gRounds_internal` 并组合 `one_round_inv_correct` 三次

### 依赖

- `one_round_inv_correct`（已证明）
